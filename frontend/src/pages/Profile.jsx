...
const Profile = () => {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const res = await api.put('/user/password', passwordData);
  ...
  // If user is somehow null, avoid crashing
  if (!user) return null;
      ...
      <div className="bg-slate-900/40 border border-white/5 rounded-3xl overflow-hidden backdrop-blur-sm shadow-xl mb-8 relative">
        ...
        <div className="px-8 pb-8 sm:flex sm:items-end sm:gap-6 relative -mt-12">
          ...
          <div className="mt-4 sm:mt-0 pt-2 flex-1">
            <h2 className="text-3xl font-bold text-white tracking-tight mb-2">{user.name}</h2>
            <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400">
              ...
              <div className="flex items-center gap-2">
                <Building2 className="w-4 h-4 text-slate-500" /> {user.group || user.department || 'ISTA NTIC Tangier'}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-8">
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input type="text" defaultValue={firstName} className="w-full bg-black/50 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500 transition-all" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Nom</label>
                <input type="text" defaultValue={lastName} className="w-full bg-black/50 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500 transition-all" />
              </div>
            </div>
            
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Email (Lecture Seule)</label>
              <input type="email" defaultValue={user.email} readOnly className="w-full bg-slate-950/50 border border-white/5 rounded-xl py-3 px-4 text-sm text-slate-600 cursor-not-allowed" />
            </div>
        ...
        <div className="bg-slate-900/40 border border-white/5 rounded-3xl p-8 backdrop-blur-sm shadow-xl">
          ...
          <div className="space-y-4">
              ...
              <div className="bg-black/20 rounded-2xl border border-white/5 overflow-hidden transition-all">
                <div className="flex items-center justify-between p-5 group hover:border-white/10 transition-colors cursor-pointer" onClick={() => setIsEditingPassword(!isEditingPassword)}>
                  <div className="flex items-center gap-4">
                  ...
                  <div className="p-5 border-t border-white/5 bg-black/40">
                    <form className="space-y-4" onSubmit={handlePasswordChange}>
                      <div className="space-y-2">
                        ...
                        <input type="password" required value={passwordData.current_password} onChange={(e) => setPasswordData({...passwordData, current_password: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500 transition-all" />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Nouveau mot de passe</label>
                          <input type="password" required minLength="6" value={passwordData.new_password} onChange={(e) => setPasswordData({...passwordData, new_password: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500 transition-all" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Confirmer nouveau mot de passe</label>
                          <input type="password" required minLength="6" value={passwordData.new_password_confirmation} onChange={(e) => setPasswordData({...passwordData, new_password_confirmation: e.target.value})} className="w-full bg-slate-950 border border-white/10 rounded-xl py-3 px-4 text-sm text-white focus:outline-none focus:border-purple-500 transition-all" />
                        </div>
                      </div>
                      <div className="pt-2 flex justify-end">
                        <button type="submit" disabled={isSubmittingPassword} className="px-6 py-2.5 bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-white rounded-xl text-sm font-bold transition-all shadow-lg active:scale-95">
                          {isSubmittingPassword ? 'En cours...' : 'Changer le mot de passe'}
                        </button>
                      </div>
                    </form>
                  </div>
        ...
        <div>
          <button 
            ...
            className="w-full flex items-center justify-center gap-2 py-4 bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white rounded-3xl text-sm font-bold transition-all border border-rose-500/20 group"
          >
