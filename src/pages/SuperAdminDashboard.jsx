...
import { 
  ...
  ShieldCheck, Database, Search, FileText, Terminal, ArrowUpDown, Filter
} from 'lucide-react';
...
const PLATFORM_STATS = [
  ...
  { label: "Charge BDD (MySQL)", value: "24%", status: "Stable", icon: <Database className="w-5 h-5 text-purple-400" />, color: "bg-purple-500/10 border-purple-500/20" },
];

const PENDING_SCHOOLS = [
  { id: 1, name: "ISTA Ibn Marfil", city: "Tanger", director: "M. Alaoui", phone: "+212 600 11 22 33", doc: "autorisation_ibnmarfil.pdf", status: "Attente Visite Physique" },
];

const ACTIVE_SCHOOLS = [
  { id: 2, name: "ISTA NTIC Tanger", city: "Tanger", users: 1240, status: "Active", joined: "2025-09-01" },
];
...
const GLOBAL_LOGS = [
  ...
  { id: 102, date: "2026-03-18 12:15:22", level: "WARNING", target: "Système Central", action: "Charge CPU Élevée", details: "Pic à 85% sur le cluster principal", ip: "Server-01" },
  ...
  { id: 104, date: "2026-03-16 23:45:00", level: "ERROR", target: "Non identifié", action: "Tentative Intrusion", details: "Échec brute-force route /admin", ip: "45.133.22.11" },
...
const SuperAdminDashboard = () => {
  ...
  const processedLogs = useMemo(() => {
    let filtered = GLOBAL_LOGS.filter(log => {
    ...
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans flex selection:bg-cyan-500 selection:text-white">
      ...
      <main className="flex-1 lg:ml-64 p-4 lg:p-8 overflow-y-auto h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#020617] to-[#020617]">
        ...
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="w-10 h-10 rounded-full bg-slate-800 border-2 border-slate-700 flex items-center justify-center font-bold text-white text-sm cursor-pointer hover:opacity-80" onClick={() => window.location.href = '/'}>
            SA
          </div>
        </header>
                  ...
                  pendingSchools.map((school) => (
                    <div key={school.id} className="bg-black/40 border border-slate-800 rounded-2xl p-6 flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-center hover:border-cyan-500/30 transition-colors">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto border-t xl:border-t-0 xl:border-l border-slate-800 pt-6 xl:pt-0 xl:pl-6">
                        <button className="flex-1 xl:flex-none px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2"><XCircle className="w-4 h-4 text-rose-400" /> Rejeter</button>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-white/[0.02] border-b border-slate-800 text-xs font-bold text-slate-500 uppercase tracking-wider">
                          <SortableHeader label="Horodatage" sortKey="date" />
                          <SortableHeader label="Niveau" sortKey="level" />
                          <SortableHeader label="Cible / École" sortKey="target" />
                          <SortableHeader label="Action Système" sortKey="action" />
                          <SortableHeader label="Adresse IP" sortKey="ip" align="right" />
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800/50">
                          ...
                          {processedLogs.length === 0 ? (
                            ...
                            processedLogs.map((log) => (
                              <motion.tr key={log.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="hover:bg-white/[0.02] transition-colors font-mono text-sm">
                                <td className="p-4 pl-4 text-slate-400 text-xs">{log.date}</td>
                                <td className="p-4">
                                ...
                                <td className="p-4 text-slate-300 truncate max-w-[150px]">{log.target}</td>
                                <td className="p-4">
